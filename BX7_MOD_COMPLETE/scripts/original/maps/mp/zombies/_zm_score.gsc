...
{
    level.score_cf_info = [];
    score_cf_register_info( "damage", 1, 1 );       // Kept! Scaled to 1 bit to save space
    score_cf_register_info( "death_normal", 1, 7 );  // 3 bits (Allows up to 7 simultaneous kills)
    score_cf_register_info( "death_torso", 1, 3 );   // 2 bits
    score_cf_register_info( "death_neck", 1, 3 );    // 2 bits
    score_cf_register_info( "death_head", 1, 3 );    // 2 bits
    score_cf_register_info( "death_melee", 1, 3 );   // 2 bits

    if ( !level.createfx_enabled )
        registerclientfield( "toplayer", "score_cf_double_points_active", 1, 1, "int" );
}

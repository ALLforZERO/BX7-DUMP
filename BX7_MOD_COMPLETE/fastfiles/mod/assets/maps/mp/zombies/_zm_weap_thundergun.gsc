...
thundergun_get_enemies_in_range()
{
    ...
    targets = get_round_enemy_array();
    
    // Add Denizens/Screechers
    ai_all = getaiarray("axis");
    foreach(ai in ai_all)
    {
        if( isDefined(ai.isscreecher) && ai.isscreecher )
        {
            targets[targets.size] = ai;
        }
    }
    zombies = get_array_of_closest( view_pos, targets, undefined, undefined, level.zombie_vars["thundergun_knockdown_range"] );

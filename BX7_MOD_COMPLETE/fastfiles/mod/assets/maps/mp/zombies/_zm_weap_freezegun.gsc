...
init()
{
    ...
    {
        level.use_winter_howl_clientfields = true;
    ...
    {
        level.use_winter_howl_clientfields = false;
    }

    set_zombie_var( "freezegun_cylinder_radius",                140 );
    set_zombie_var( "freezegun_inner_range",                    60 );
    set_zombie_var( "freezegun_outer_range",                    400 );
    set_zombie_var( "freezegun_inner_damage",                   1000 );
    set_zombie_var( "freezegun_outer_damage",                   500 );
    set_zombie_var( "freezegun_shatter_range",                  120 );
    set_zombie_var( "freezegun_shatter_inner_damage",           500 );
    set_zombie_var( "freezegun_shatter_outer_damage",           250 );
    set_zombie_var( "freezegun_cylinder_radius_upgraded",       160 );
    set_zombie_var( "freezegun_inner_range_upgraded",           120 );
    set_zombie_var( "freezegun_outer_range_upgraded",           700 );
    set_zombie_var( "freezegun_inner_damage_upgraded",          50000 );
    set_zombie_var( "freezegun_outer_damage_upgraded",          750 );
    set_zombie_var( "freezegun_shatter_range_upgraded",         150 );
    set_zombie_var( "freezegun_shatter_inner_damage_upgraded",  750 );
    set_zombie_var( "freezegun_shatter_outer_damage_upgraded",  500 );
    

    level._effect[ "freezegun_shatter" ]                = LoadFX( "weapon/zmb_staff/fx_zmb_staff_ice_exp" );
    level._effect[ "freezegun_crumple" ]                = LoadFX( "weapon/zmb_staff/fx_zmb_staff_ice_exp" );
    ...
    level._effect[ "freezegun_upgraded" ]               = LoadFX( "weapon/zmb_staff/fx_zmb_staff_ice_exp" );
...
freezegun_do_damage( upgraded, player, dist_ratio )
{
    ...
    percent_multiplier = ( level.round_number < 20 ) ? 0.51 : 0.36;
    
    damage = Int( self.maxhealth * percent_multiplier );
...
{
    if ( level.use_winter_howl_clientfields )
    {
        self SetClientField( "freezegun_extremity_damage_fx", 1 );
    }
    else 
    {
        PlayFXOnTag( level._effect["freezegun_damage_sm"], self, "j_spineupper" );
    }
}

freezegun_clear_extremity_damage_fx()
{
    // self SetClientField( "freezegun_extremity_damage_fx", 0); // REMOVED
    // Note: In GSC, you can't "delete" an FX easily once played, 
    // but since these are short bursts or death effects, it's fine.
}
...
freezegun_set_torso_damage_fx()
{
    ...
    {
        playfxontag( level._effect["freezegun_damage_torso"], self, "J_SpineLower" );
    }
}

freezegun_clear_torso_damage_fx()
{
    // self SetClientField( "freezegun_torso_damage_fx", 0); // REMOVED
}
...
freezegun_death( hit_location, hit_origin, player )
{
    if ( self.isdog )
    {
        ...
        self DoDamage( self.health + 1000, self.origin, player ); 
        return;
    }
...
freezegun_zombie_damage_response( mod, hit_location, hit_origin, player, amount )
{
    ...
    {
	    if ( isPlayer( player ) && player maps\mp\zombies\_zm_powerups::is_insta_kill_active() )
        {
            self.health = 0;
            self DoDamage( self.health + 666, self.origin, player, player, "none", "MOD_UNKNOWN", 0, "freezegun_zm" );
            return true; 
        }
        if ( !isDefined( self.pre_freeze_speed ) )
        {

            self.pre_freeze_speed = (self.zombie_move_speed == "walk") ? "run" : self.zombie_move_speed;
        }

        self thread freezegun_damage_response( player, amount );
        self thread freezegun_thaw_timer( self.pre_freeze_speed );
...
freezegun_thaw_timer( original_speed )
{
    self endon( "death" );
    self notify( "stop_freezegun_thaw" );
    self endon( "stop_freezegun_thaw" );

    wait( 5.0 );

    if ( isDefined( self ) && !is_true( self.freezegun_death ) )
    {
        self set_zombie_run_cycle( original_speed );
        self freezegun_clear_extremity_damage_fx();
        
        self.pre_freeze_speed = undefined; 
    }
}

...
CoD.CompetitiveScoreboard.ScorePrefixes = {
    -- Caught via Client Field updates
    ["score_cf_damage"]             = "", -- Empty = numbers only
    ["score_cf_death_normal"]       = " Zombie Kill",
    ["score_cf_death_torso"]        = " Zombie Kill",
    ["score_cf_death_neck"]         = " Zombie Kill",
    ["score_cf_death_head"]         = " Headshot",
    ["score_cf_death_melee"]        = " Melee",

    -- Caught via general engine score loop updates
    ["generic_score"]               = "", -- Empty = numbers only
    ["damage_score"]                = ""  -- Empty = numbers only
}
...
CoD.CompetitiveScoreboard.ClientFields.score_cf_damage = {
    min = 0,
    max = 1, -- Matches the safe 1-bit allocation
    scoreScale = 10
}
...
LUI.createMenu.CompetitiveScoreboard = function (LocalClientIndex)
	...
	for ClientIndex = 1, CoD.CompetitiveScoreboard.TeamPlayerCount, 1 do
		...
		for ClientIndex = 1, CoD.CompetitiveScoreboard.ClientFieldMaxValue, 1 do
			...
			FloatingScoreText:setFont(CoD.fonts[CharacterNameFont])
			...
			-- === RESTORED PARENT CONTAINER FOR YOUR COORDINATES ===
			CompetitiveScoreboardWidget.scaleContainer:addElement(FloatingScoreText)
...
CoD.CompetitiveScoreboard.Update = function (CompetitiveScoreboardWidget, ClientInstance)
    ...
    if #ClientInstance.competitivescores <= #CompetitiveScoreboardWidget.Scores then
        for ClientIndex, LocalClient in pairs(ClientInstance.competitivescores) do
            ...
            -- RESET: The logic here is fine, no changes needed to ScoreAmount logic
            PlayerScoreListWidget.currentClientFieldScore = 0
            ...
            -- SAVING YOUR ID: We identify "Self" here, so we save your clientNum globally
            if ClientIndex == ClientInstance.selfindex then
                CoD.CompetitiveScoreboard.MyClientNum = LocalClient.clientNum
...
CoD.CompetitiveScoreboard.Update_ClientFields_FlyingScore = function (CompetitiveScoreboardWidget, ClientInstance)
    ...
    elseif ClientInstance.name == CoD.CompetitiveScoreboard.DoublePointsActive_ClientFieldName then
        ...
        local eventName = ClientInstance.name 

        -- FILTER: Use our new global variable check
        if ClientInstance.entNum == CoD.CompetitiveScoreboard.MyClientNum then
            if eventName ~= "score_cf_damage" then
                -- ENHANCEMENT: Calculate total accumulated value instantly instead of looping rows
                local TotalAccumulatedScore = ScoreAmount * MaxClientFields
                CoD.CompetitiveScoreboard.FloatingScoreStart(PlayerScoreListWidget, TotalAccumulatedScore, eventName, MaxClientFields)
            end
        end
...
CoD.CompetitiveScoreboard.FloatingScoreStart = function (PlayerScoreListWidget, ScoreAmount, eventName, multiplierCount)
    
    if UIExpression.DvarString(nil, "ui_scorefeed_enabled") == "0" then 
        return 
    end
    
    local CompetitiveScoreboardWidget = PlayerScoreListWidget:getParent():getParent()
    if not CompetitiveScoreboardWidget then return end
    ...
    if PlayerScoreListWidget.scoreFeed == nil then
        PlayerScoreListWidget.scoreFeed = LUI.UIVerticalList.new({
            left = boxLeft, top = boxTop, right = boxLeft + boxWidth, bottom = boxTop + 100,
            leftAnchor = false, topAnchor = false, rightAnchor = false, bottomAnchor = false
        })
        PlayerScoreListWidget.scoreFeed.activeRows = {}
        CompetitiveScoreboardWidget:addElement(PlayerScoreListWidget.scoreFeed)
    end

    local feed = PlayerScoreListWidget.scoreFeed
    local maxRows = 7
    local expandTime = 200
    local rowHeight = 22

    -- === ANIMATION SETUP ===
    local RowWidget = LUI.UIElement.new({
        left = 0, top = 0, right = boxWidth, bottom = 0,
        leftAnchor = true, topAnchor = true, rightAnchor = false, bottomAnchor = false,
        alpha = 0 -- Start invisible
    })
    
    feed:addElement(RowWidget)
    table.insert(feed.activeRows, RowWidget)

    -- Define the states
    RowWidget:registerAnimationState("appear", { alpha = 1, top = 0, bottom = rowHeight })
    RowWidget:registerAnimationState("fade", { alpha = 0 })
    RowWidget:registerEventHandler("transition_complete_fade", RowWidget.close)
    ...
    scoreText:setFont(CoD.fonts["Default"])

    local suffixText = ""
    if eventName and CoD.CompetitiveScoreboard.ScorePrefixes[eventName] then
        suffixText = CoD.CompetitiveScoreboard.ScorePrefixes[eventName]
    end

    -- ENHANCEMENT: Formats the multiplier if more than 1 kill registered on this tick
    local multiplierStr = ""
    if multiplierCount and multiplierCount > 1 then
        multiplierStr = string.format(" (x%d)", multiplierCount)
    end

    if ScoreAmount > 0 then
        scoreText:setText(string.format("+%d%s%s", ScoreAmount, suffixText, multiplierStr))
        scoreText:setRGB(0.9, 0.9, 0)
    else
        scoreText:setText(string.format("%d%s%s", ScoreAmount, suffixText, multiplierStr))
        scoreText:setRGB(CoD.CompetitiveScoreboard.FloatingLosePointsColor.r, CoD.CompetitiveScoreboard.FloatingLosePointsColor.g, CoD.CompetitiveScoreboard.FloatingLosePointsColor.b)
    end
    RowWidget:addElement(scoreText)

    -- === SMOOTH ANIMATION ===
    RowWidget:animateToState("appear", expandTime, true, true)
    
    RowWidget.fadeTimer = LUI.UITimer.new(1200, "start_feed_fade", true)
    RowWidget:addElement(RowWidget.fadeTimer)
    RowWidget:registerEventHandler("start_feed_fade", function(element)
        element:animateToState("fade", 300, true, true)
    end)

    if maxRows < #feed.activeRows then
        local oldestRow = feed.activeRows[1]
        if oldestRow then oldestRow:animateToState("fade", expandTime, true, true) end
        table.remove(feed.activeRows, 1)
    end
end
...
CoD.CompetitiveScoreboard.FloatingTextFlyingFinish = function (FloatingScoreText, ClientInstance)
	if ClientInstance.interrupted ~= true then
        FloatingScoreText.isUsed = false
        FloatingScoreText:setLeftRight(true, false, 0, CoD.CompetitiveScoreboard.RowWidth)
        FloatingScoreText:setTopBottom(true, false, 0, CoD.CompetitiveScoreboard.RowHeight)
    end
end

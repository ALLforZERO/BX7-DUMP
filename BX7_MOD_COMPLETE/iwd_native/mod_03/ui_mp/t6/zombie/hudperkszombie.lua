...
local function HUDToggleIcon(widget, event)
	print("[^2LUA^7] [^5DEBUG^7] Debugging hud_toggle_icon event")
	...
	if CoD.ADN.HUDIcons[clientid] == nil then
		CoD.ADN.HUDIcons[clientid] = {}
	end

	if visible then
		table.insert(CoD.ADN.HUDIcons[clientid], icon)
	else
		for index, activeIcon in ipairs(CoD.ADN.HUDIcons[clientid]) do
			if icon == activeIcon then
				table.remove(CoD.ADN.HUDIcons[clientid], index)
				break
			end
		end
	end

	print("[^2LUA^7] [^5DEBUG^7] clientid: " .. tostring(clientid))
	print("[^2LUA^7] [^5DEBUG^7] visible: " .. tostring(visible))
	print("[^2LUA^7] [^5DEBUG^7] icon: " .. tostring(icon))
	-- for k, v in pairs(event) do
	-- 	print("  k = (" .. type(k) .. ") " .. tostring(k))
	-- 	print("  v = (" .. type(v) .. ") " .. tostring(v))
	-- end
end
...
LUI.createMenu.PerksArea = function (LocalClientIndex)
	...
	local _, _, _, SafeBottom = Engine.GetUserSafeAreaForController(LocalClientIndex)

	local BottomPadding = 6
	local Y = -CoD.Perks.IconSize - BottomPadding
	Y = Y + SafeBottom

	PerksAreaWidget.scaleContainer = CoD.SplitscreenScaler.new(nil, CoD.Zombie.SplitscreenMultiplier)
	PerksAreaWidget.scaleContainer:setLeftRight(false, false, 0, 0)
	PerksAreaWidget.scaleContainer:setTopBottom(false, false, Y, Y)
	...
	for ClientFieldIndex = 1, #CoD.Perks.ClientFieldNames, 1 do
		...
		Widget:setTopBottom(false, true, 0, CoD.Perks.IconSize)
		Widget:setScale(1)
		Widget.active = false
		Widget.perkId = nil
	...
	PerksAreaWidget:registerEventHandler("adn_icon", HUDToggleIcon)
...
CoD.Perks.UpdateVisibility = function (Menu, ClientInstance)
    ...
    local _, _, _, SafeBottom = Engine.GetUserSafeAreaForController(f2_local0)

    local BottomPadding = 6
    local Y = -CoD.Perks.IconSize - BottomPadding
    Y = Y + SafeBottom

    Menu.scaleContainer:setTopBottom(false, false, Y, Y)

    -- Removed BIT_UI_ACTIVE check so HUD icons remain visible while popups/menus are open
    if UIExpression.IsVisibilityBitSet(f2_local0, CoD.BIT_HUD_VISIBLE) == 1 
       ...
       and UIExpression.IsVisibilityBitSet(f2_local0, CoD.BIT_AMMO_COUNTER_HIDE) == 0 
       ...
       and (not CoD.IsShoutcaster(f2_local0) or CoD.ExeProfileVarBool(f2_local0, "shoutcaster_scorestreaks") and Engine.IsSpectatingActiveClient(f2_local0)) 
       and CoD.FSM_VISIBILITY(f2_local0) == 0 then
...
CoD.Perks.RemovePerkIcon = function (Menu, OwnedPerkIndex)
	...
	for PerkIndex = OwnedPerkIndex, #CoD.Perks.ClientFieldNames, 1 do
		...
		if PerkWidget.meterContainer then
			PerkWidget.m_eventHandlers["vulture_perk_disease_meter"] = nil
			PerkWidget.meterContainer:close()
			PerkWidget.meterContainer = nil
		end

		if not NextPerkWidget then
			Menu.scaleContainer:removeElement(PerkWidget)
			PerkWidget.active = false
...
CoD.Perks.Update = function (Menu, ClientInstance)
	...
	for OwnedPerkIndex = 1, #CoD.Perks.ClientFieldNames, 1 do
		...
		if ClientInstance.newValue == CoD.Perks.STATE_OWNED then
			if not PerkWidget.perkId then
				...
				if not PerkWidget.active then
					Menu.scaleContainer:addElement(PerkWidget)
					PerkWidget.active = true
				end
	...
	local activePerks = 0
	for PerkIndex = 1, #CoD.Perks.ClientFieldNames do
		if Menu.perks[PerkIndex].active then
			activePerks = activePerks + 1
		end
	end

	local width = (CoD.Perks.IconSize + CoD.Perks.Spacing) * activePerks - CoD.Perks.Spacing
	Menu.scaleContainer:setLeftRight(false, false, -width / 2, width / 2)
end
...
CoD.Perks.AddVultureMeter = function (Menu, PerkWidget)
	-- if Menu.vultureMeter and PerkWidget.perkId == 'perk_vulture' then
	-- 	local parent = Menu.vultureMeter:getParent()
	-- 	if parent then
	-- 		parent.meterContainer = nil
	-- 		parent:removeElement(Menu.vultureMeter)
	-- 	end

	-- 	PerkWidget.meterContainer = Menu.vultureMeter
	-- 	PerkWidget:addElement(Menu.vultureMeter)
	-- 	return
	-- end
	if not PerkWidget.meterContainer and PerkWidget.perkId == 'perk_vulture' then
		...
		Menu.vultureMeter = meterContainer
	end
end
